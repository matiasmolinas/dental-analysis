"""The execution-gap directive harness — the project's generalizable input-improvement capability.

The one clean, repeated win (the missing-data directive) raised guardrail pass 0->1.0 — while a
free-form model handed the SAME directive as prose fell back to 0.00. The payoff channel is the
EXECUTION GAP: content the model KNOWS (`K_know`) but does not reliably DEPLOY in situ (`K_exec`). A
deterministic, ENFORCED directive externalizes exactly `K_know \ K_exec`; a semantic directive the
model already applies adds nothing (and can dilute).

This module makes that a repeatable, measured capability:
  ...
  2. a 3-arm A/B — base / enforced / prose-handoff — that measures the effect (bootstrap CI) and
     classifies WHY it paid (execution gap vs knowledge gap vs no effect); the prose arm isolates the
     value of *enforcement* from the value of the content.

Model-agnostic (injected callables) → testable offline; the live wiring uses the Anthropic SDK.
Non-diagnostic (scores structural/guardrail metrics, never a patient inference).
"""
...
def predict_pays(known: bool, deterministic: bool, dropped_in_situ: bool) -> str:
    """Classify a candidate directive BEFORE any A/B. The single strongest,
    ...
    if not deterministic:
        return "predict_null_semantic"       # known + applied → no effect (may hurt, cf. F1)
...
def stated_but_dropped(
    ...
    step blind (→ known ∈ K_know); `solve_fn(task)` runs the real solve and `deploys_check` tests
    whether the step was actually deployed (→ ∈ K_exec). Returns the three flags + the prediction."""
...
def run_directive_ab(
    ...
    model-judged quality score (a model grading its own output is not an external signal)."""
    per_case = []
    ...
    ci_vs_base = bootstrap_ci([c["enforced"] - c["base"] for c in per_case])
    ci_vs_prose = bootstrap_ci([c["enforced"] - c["prose"] for c in per_case])
