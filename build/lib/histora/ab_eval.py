"""Scoring + A/B evaluation for the non-diagnostic relational output.

The apparatus's scoring layer: given an `output_schema.json`-shaped analysis, it measures
mechanism-recall (CV + neuro mediators), relational-recall (mediators used inside a *traced*
axis, not merely named), missing-data flagging, traceability, and the protected non-diagnostic
guardrail. `build_inputs(record)` produces the converged input — the deterministic structural
signals + the missing-mediator collection directive (W1) injected on top of the glossed record.

Model-agnostic: `run_ab(records, model_fn)` takes a `model_fn(prompt)->dict` so it is testable
offline with a stub; the live agent (`histora.agent`) supplies a real Claude call. Scoring is
guardrail-aware — an output that fails the guardrail is not deployable regardless of its accuracy.
"""
