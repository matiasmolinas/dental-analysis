...
The **spec** the scoring layer measures the analysis against: the mediating concepts that must be
represented for the model to be *relating* oral and systemic data, not merely listing it. A mediator
counts only when it is used inside a traced relational axis (see `histora.ab_eval`).

Each concept maps to several candidate surface forms so recall can be matched flexibly to how the
model happens to name the concept.
