...
## 6. What we deliberately set aside, and why (rigor, briefly)

In the course of building HISTORA the team also investigated a separate, more speculative idea:
whether reading a model's *internal* "workspace" — an interpretability signal in the spirit of
global-workspace / "Jacobian lens" work — could be used to optimize the agent's own inputs. This
was pursued with a full, honest apparatus (bootstrap-CI ablations, a model-agnostic A/B scorer, a
promotion gate) and it produced a **rigorous negative with a precise explanation**: the internal
signal is genuinely *non-redundant* with the model's output, but no actuator built here converted
it into an outcome gain over a strong blind baseline — because a competent reader already has,
from the output itself, what the lens would add on this largely re-derivable task.

We report this plainly, and briefly, because it is evidence of the project's methodological
standards: the discipline that produced a clean negative on the lens is the same discipline that
produced the honestly-caveated *positive* on the science. **And the durable value — the
mechanistic pipeline, the ε calibration to the ~0.5 mg/L ΔCRP anchor, and the validated NHANES
oral→cognition result — depends on none of it.** That product is the vision, and it stands on its
own.
